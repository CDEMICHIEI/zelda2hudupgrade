...
--
--
-- -- Stole all of this font code from Metroid LUA Script by Neill Corlett
...
local function gdMonoTile(ofs)
    return gdTile(ofs,"\127\0\0\0","\0\255\255\255","\0\0\40\255","\0\255\40\0")
end

local function gdMonoTileRed(ofs)
    return gdTile(ofs,"\127\0\0\0","\0\255\255\255","\0\255\40\0","\0\0\40\255")
end
...
local Z2Font = {
   ...
   -- [string.byte("?")] = gdMonoTile(0x1B8B0),
    ...
    [string.byte(";")] = gdMonoTileRed(0x021FB0), -- JAR
...
-- End Metroid LUA code
--------------------------------------
readByte = memory.readbyte
prevLife = math.ceil(memory.readbyte(0x0774)/2)
DamTimer =0
...
local rawLife = memory.readbyte(0x0774)
local nowLife = math.ceil(rawLife/2)
local rawMagic = memory.readbyte(0x0773)
local Magic = math.ceil(rawMagic/2)
local Keys = memory.readbyte(0x0793)
local Lives = memory.readbyte(0x0700)
...
--gui.text(176, 08, "K:"..Keys,'orange',clear)
DrawZ2Font(176,08,"$*"..Keys)
--gui.text(192, 08, "L:"..Lives,'green',clear)
DrawZ2Font(220,216,"@*"..Lives)
-- equipped magic
--local equipMagic = readByte(0x03C0)
--local magicCost = readByte(0xB7A9+equipMagic)

-- List enemy's hp
for i=0,5 do
	local enHP = readByte(0x00C7 - i)
	if enHP ~= 0 and enHP <200 then
		--gui.text(0,215-(8*i), i..' '..enHP, 'white',clear)
...
local dropCountL = readByte(0x05E0)
for i=0,5 do
	if i < dropCount then
	dropColor = 'green'
	dropChar = "+"
	...
	--gui.text(176+i*8, 24, ""..dropChar,dropColor,clear)
	DrawZ2Font(176+i*8, 24, dropChar)
	if i < dropCountL then
dropColor = 'green'
	dropChar = "+"
	else
	dropColor = 'red'
	...
	--gui.text(176+i*8, 32, ""..dropChar,dropColor,clear)
...
local spellReady = readByte(0x0F4A)
...
if DamTimer > 0 then
	...
	local yUp = DamTimer / 12
	--gui.text(LinkX,LinkY-12+yUp, Damage,'red', clear)
...
--gui.text(readByte(0x0804),readByte(0x0805),readByte(0x0804)..','..readByte(0x0805),'white',0)
